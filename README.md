...
* Hiding and unhiding elements using `element.classList.add('hide')` or `element.classList.remove('hide')`, respectively, which I have used on multiple locations in the JS code, as well as adding `.hide {display: none;}` in CSS
